...
bool HttpRangeClient::heartbeat(
  ...
  for (std::size_t index = 0;
       index < heartbeat.gpus.size();
       ++index) {
    const auto& gpu =
        heartbeat.gpus[index];
