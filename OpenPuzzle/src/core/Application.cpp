...
int Application::run(int argc,char** argv){ std::vector<std::string> args; for(int i=1;i<argc;++i) args.emplace_back(argv[i]); if(args.empty()){ std::cout<<"OpenPuzzle 0.11.1-dev\n"; return 0;} auto cmd=args[0]; std::vector<std::string> r(args.begin()+1,args.end()); try{ if(cmd=="init")return cmdInit(); if(cmd=="import-puzzle-json")return cmdImportPuzzleJson(r); if(cmd=="list-puzzles")return cmdListPuzzles(); if(cmd=="create-job")return cmdCreateJob(r); if(cmd=="list-ranges")return cmdListRanges(r); if(cmd=="complete-job")return cmdCompleteJob(r); if(cmd=="stats")return cmdStats(r); if(cmd=="configure-tool")return cmdConfigureTool(r); if(cmd=="tools")return cmdTools(); if(cmd=="gpu-list")return cmdGpuList(); if(cmd=="gpu-select")return cmdGpuSelect(r); if(cmd=="bitcrack-command")return cmdBitcrackCommand(r); if(cmd=="start-job")return cmdStartJob(r); if(cmd=="process-test")return cmdProcessTest(r); if(cmd=="execution-test")return cmdExecutionTest(r); if(cmd=="parse-bitcrack-line")return cmdParseBitCrackLine(r); if(cmd=="dashboard")return cmdDashboard(r); if(cmd=="audit")return cmdAudit(r);}catch(const std::exception&e){ std::cerr<<"Error: "<<e.what()<<"\n"; return 1;} std::cerr<<"Unknown command\n"; return 1; }
