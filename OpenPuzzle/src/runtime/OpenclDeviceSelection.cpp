...
int parseDevice(const std::string& token) {
  const auto value = trim(token);
  if (value.empty()) {
    throw std::runtime_error(
        "OpenCL device list contains an empty item");
  }
  ...
  try {
    ...
    throw std::runtime_error(
        "OpenCL devices must be non-negative whole numbers");
  }
...
std::vector<int> inventory(
  ...
  for (const auto& gpu : availableDevices) {
    if (gpu.device < 0) {
      throw std::runtime_error(
          "OpenCL inventory contains an invalid device index");
    }
    if (!seen.insert(gpu.device).second) {
      throw std::runtime_error(
          "OpenCL inventory contains duplicate device " +
...
std::vector<int> OpenclDeviceSelection::resolve(
    const std::string& selector,
    const std::vector<GpuInfo>& availableDevices) {
  const auto available = inventory(availableDevices);
  if (available.empty()) {
  ...
  while (beginning <= requested.size()) {
    const auto separator = requested.find(',', beginning);
    ...
    if (!seen.insert(device).second) {
      throw std::runtime_error(
          "OpenCL device " + std::to_string(device) +
          " was selected more than once");
    }
    if (!std::binary_search(
            available.begin(), available.end(), device)) {
      throw std::runtime_error(
          "OpenCL device " + std::to_string(device) +
          " was not found");
    }
